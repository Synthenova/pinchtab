...
func (h *Handlers) HandleOpenAPI(w http.ResponseWriter, _ *http.Request) {
	...
	httpx.JSON(w, 200, map[string]any{
		...
		"paths": map[string]any{
			...
			"/profiles/cloud/discover":           map[string]any{"post": map[string]any{"summary": "Discover cloud-backed profiles in a GCS bucket prefix"}},
			"/profiles/cloud/import":             map[string]any{"post": map[string]any{"summary": "Import a discovered cloud-backed profile into local PinchTab"}},
			...
			"/profiles/{id}/cloud/finalize":      map[string]any{"get": map[string]any{"summary": "Get cloud finalize/upload status after stop"}},
			"/profiles/{id}/cloud/retry-upload":  map[string]any{"post": map[string]any{"summary": "Retry a failed cloud upload without relaunching the browser"}},
			"/profiles/{id}/cloud/discard-local": map[string]any{"post": map[string]any{"summary": "Discard unsynced local cloud profile changes and release the lease"}},
